...
# --- SPEECH TO TEXT TRANSLATION ---
    ...
    audio_file = st.file_uploader("Upload an audio file (mp3/wav/ogg/m4a):", type=["wav", "mp3", "m4a", "ogg"])
    ...
    if audio_file:
        ...
        try:
            with st.spinner("Transcribing..."):
                ...
                translated = translator.translate(recognized_text, dest=target_languages[target_language]).text
                st.success("Translated Text:")
                st.markdown(f"<p style='font-size: 20px; color: #4CAF50;'>{translated}</p>", unsafe_allow_html=True)

                st.download_button("Download Transcribed Text", recognized_text, file_name="speech_transcript.txt")
                st.download_button("Download Translated Text", translated, file_name="translated_text.txt")
        except Exception as e:
